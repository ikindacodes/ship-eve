/**
 * Shared library code (`lib/`) — import-only, not an eve authored slot.
 *
 * Helpers here are imported by tools at build time. eve does not mount `lib/`
 * into the model workspace; only `skills/` and `sandbox/workspace/` land there.
 */
...
function scoreArticle(article: DocArticle, terms: string[]): number {
  const haystack =
    `${article.title} ${article.excerpt} ${article.id}`.toLowerCase();
  return terms.reduce(
